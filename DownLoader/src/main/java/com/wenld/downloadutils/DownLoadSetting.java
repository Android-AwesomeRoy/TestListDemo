...
public class DownLoadSetting {
    ...
    public static void changeFileDir(String fileDir) {
        Log.d("DownloadSetting", "fileDir == " + fileDir);
        if (fileDir == null) throw new NullPointerException("无效的地址");
        if (!new File(fileDir).exists() || new File(fileDir).exists()) {
    ...
    public static void setFileMaxNum(int maxNum) {
        if (maxNum < 1) throw new NullPointerException("the num not less than 1");
    ...
    public static void setSsl(InputStream[] certificates, InputStream bksFile, String password) {
        HttpUtils.getInstance()
                 .setSslParams(SSLParams.getSslSocketFactory(certificates, bksFile, password));
    }
